    ...
    <div className="min-h-screen flex flex-col bg-bg">
      ...
      <main className="flex-grow pt-32 pb-40 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        >
          ...
          <h1 className="text-5xl font-display mb-12 text-text">Your Collection</h1>

          {cart.length === 0 ? (
            <div className="glass p-20 rounded-[3rem] text-center border border-black/5">
              <div className="w-20 h-20 bg-black/5 rounded-full flex items-center justify-center mx-auto mb-8 border border-black/10">
                <ShoppingBag className="text-text/20" size={32} />
              </div>
              <h2 className="text-2xl font-display mb-4">Your vault is empty</h2>
              <p className="text-text/40 mb-12 max-w-md mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <AnimatePresence>
                  {cart.map((item) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, x: 20 }}
                      className={`glass p-4 sm:p-6 rounded-3xl flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-6 border transition-all duration-700 ${item.stock <= 0 ? 'border-red-900/30 opacity-40 grayscale-[0.8]' : 'border-black/5'}`}
                    >
                      <div className="flex items-center space-x-4 sm:space-x-6 w-full sm:w-auto">
                        <img 
                          src={item.images[0]} 
                          className="w-20 h-24 sm:w-24 sm:h-32 object-cover rounded-xl bg-black/5 shrink-0" 
                          alt={item.name} 
                        />
                        <div className="flex-grow">
                          <span className="text-[9px] sm:text-[10px] uppercase tracking-widest text-text/30">{item.brand}</span>
                          ...
                          <div className="flex flex-col mt-1 sm:mt-2">
                             {item.discount && (!item.offerExpiry || new Date(item.offerExpiry) > new Date()) ? (
                               <div className="flex items-center space-x-2">
                                 <span className="text-text/20 line-through text-[10px] font-light italic">₹{item.price.toLocaleString()}</span>
                           ...
                           {item.stock !== undefined && (
                            <div className={`text-[9px] sm:text-[10px] uppercase font-bold mt-1 sm:mt-2 flex items-center space-x-2 ${item.stock <= 0 ? 'text-red-500' : (item.stock <= 5 ? 'text-orange-500' : 'text-text/40')}`}>
                               {item.stock > 0 ? (
                                 <>
                      ...
                      <div className="flex items-center justify-between w-full sm:w-auto sm:flex-grow-0 sm:space-x-4 border-t sm:border-t-0 border-black/5 pt-4 sm:pt-0">
                        <div className="flex items-center glass rounded-full p-1 border border-black/10">
                          <button 
                        ...
                        <button 
                          ...
                          className="flex items-center space-x-2 text-text/40 hover:text-red-500 transition-colors py-2 px-4 rounded-xl hover:bg-red-500/10"
                        >
                          <Trash2 size={18} />
              ...
              <div className="lg:col-span-1">
                <div className="glass p-8 rounded-[2.5rem] border border-black/10 sticky top-32">
                  ...
                  <div className="space-y-4 mb-8 pb-8 border-b border-black/5">
                    ...
                    <div className="pb-6 border-b border-black/5">
                       <p className="text-[10px] uppercase tracking-widest text-text/40 mb-3 ml-1">Promotional Coupon</p>
                       {coupon ? (
                         <div className="flex items-center justify-between bg-gold/10 border border-gold/20 p-4 rounded-xl">
                            <div className="flex items-center space-x-3">
                               <span className="text-xs font-bold text-text uppercase tracking-widest">{coupon.code}</span>
                            </div>
                         ...
                         <div className="flex space-x-2">
                            <input 
                              ...
                              placeholder="CODE"
                              className="flex-grow bg-black/5 border border-black/10 rounded-xl px-4 py-3 outline-none focus:border-gold text-xs uppercase tracking-widest transition-all"
                            />
                            <button 
                              onClick={handleApplyCoupon}
                              disabled={isValidatingCoupon || !couponCode}
                              className="px-6 py-3 bg-black/5 hover:bg-black/10 border border-black/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-gold disabled:opacity-20 transition-all"
                            >
                  ...
                  <Link 
                    ...
                    className={`w-full py-5 font-bold uppercase tracking-widest flex items-center justify-center transition-all ${hasOutOfStock ? 'bg-black/10 text-text/50 cursor-pointer' : 'gold-gradient text-luxury-black hover:scale-[1.02] active:scale-[0.98]'}`}
                  >
                    Proceed to Checkout
        ...
        {showErrorPopup && (
          ...
          >
            ...
            >
              ...
              <p className="text-text/40 mb-10 text-[11px] uppercase tracking-widest leading-relaxed">
              ...
              <div className="space-y-4 mb-10 text-left">
                {cart.filter(item => item.stock <= 0).map(item => (
                  <div key={item.id} className="flex items-center space-x-4 p-4 border border-black/5 bg-black/[0.02]">
                    <img src={item.images[0]} className="w-12 h-12 object-cover opacity-50" />
                    <span className="text-[10px] uppercase font-bold tracking-widest text-text/60">{item.name}</span>
                  </div>
                ))}
              </div>
              ...
              <button 
                onClick={() => setShowErrorPopup(false)}
                className="w-full py-4 bg-black/5 hover:bg-black/10 text-text font-bold uppercase tracking-[0.3em] text-[10px] transition-all border border-black/10"
              >
