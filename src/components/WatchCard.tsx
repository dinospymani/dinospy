...
export default function WatchCard({ product }: WatchCardProps) {
  ...
  return (
    ...
    >
      ...
      <Link to={`/product/${product.id}`} className="block aspect-square mb-6 sm:mb-12 overflow-hidden bg-luxury-black/50 relative group-hover:shadow-[0_40px_80px_rgba(0,0,0,0.6)] transition-all duration-1000">
        <motion.img 
          layoutId={`image-${product.id}`}
          src={product.images[0]} 
          alt={product.name}
          className="w-full h-full object-contain p-2 sm:p-12 group-hover:scale-110 transition-transform duration-[2s] ease-[0.22, 1, 0.36, 1]"
        />
